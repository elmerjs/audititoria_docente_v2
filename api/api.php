...
function reconciliarObservaciones(PDO $pdo, string $periodo, array $alertasActuales): array {
    ...
    $stmtObs = $pdo->prepare("SELECT id FROM observaciones WHERE periodo = :periodo AND estado = 'ABIERTA'");
    $stmtObs->execute(['periodo' => $periodo]);
    $idsAbiertas = $stmtObs->fetchAll(PDO::FETCH_COLUMN);

    $mapaBadge = [];

    foreach ($idsAbiertas as $idObs) {
        ...
        if ($todasSubsanadas) {
            $pdo->prepare("UPDATE observaciones SET estado = 'SUBSANADA' WHERE id = :id")
                ->execute(['id' => $idObs]);
        }
        foreach ($filas as $f) {
            $clave = claveObsNormalizada($f['identificacion'], $f['codigo_materia'], $f['grupo']);
    ...
    // Observaciones que ya estaban SUBSANADA/CERRADA de antes: mantener su badge.
    $stmtOtras = $pdo->prepare("SELECT ofi.identificacion, ofi.codigo_materia, ofi.grupo, o.id, o.estado
                                 FROM observaciones_filas ofi
                                 INNER JOIN observaciones o ON o.id = ofi.id_observacion
                                 WHERE o.periodo = :periodo AND o.estado <> 'ABIERTA'");
    $stmtOtras->execute(['periodo' => $periodo]);
    foreach ($stmtOtras->fetchAll(PDO::FETCH_ASSOC) as $f) {
        $clave = claveObsNormalizada($f['identificacion'], $f['codigo_materia'], $f['grupo']);
        if (!isset($mapaBadge[$clave])) {
            $mapaBadge[$clave] = ['id_observacion' => (int)$f['id'], 'estado' => $f['estado']];
        }
    }
